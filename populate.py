...
and it will:
  ...
  2. For each instrument, fetch price history (incrementally -- only pulls
     dates newer than what's already in the DB)
...
def fetch_price_history(ticker: str, source: str, stooq_kind: str = "equity", full_refresh: bool = False):
    """
    Fetch and store price history for one ticker, incrementally. Returns
    True if any new data was written.
    """
    ...
    if source == "stooq":
        ...
        rows = stooq_fetcher.fetch(symbol)
        if rows is None:
            # fall back to yfinance if Stooq doesn't carry this symbol
            print(f"    stooq had no data for {ticker} ({symbol}), falling back to yfinance")
            rows = yfinance_fetcher.fetch_history(ticker)
    ...
    return n > 0 or since is not None  # "not stale" even if 0 new rows today
...
def run(full_refresh: bool = False):
    ...
    fetch_price_history(gold_ticker, cfg["gold_source"], full_refresh=full_refresh)
    ...
    for c in cfg.get("commodities", []):
        ...
        fetch_price_history(c["ticker"], c["source"], full_refresh=full_refresh)
    ...
    for s in cfg.get("sectors", []):
        ...
        fetch_price_history(s["ticker"], s["source"], s.get("stooq_kind", "equity"), full_refresh)
        ...
        if fund:
            db.upsert_fundamentals(s["ticker"], fund["name"], fund["pe"], fund["pb"],
                                    fund["rating"], dt.datetime.utcnow().isoformat())
    ...
    for f in cfg.get("fred_series", []):
        ...
        # FRED series are small (a few hundred monthly/quarterly points at
        # most) so we just re-pull the whole thing each run rather than
        # bothering with incremental fetch -- not worth the complexity.
        rows = fred_fetcher.fetch(f["series_id"])
        if rows:
    ...
    db.set_meta("last_full_run", dt.datetime.utcnow().isoformat())
    print("\nDone.")

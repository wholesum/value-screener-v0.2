...
3. Go to the **Web** tab → Add a new web app → Manual configuration →
   ...
   made.
4. Go to the **Tasks** tab → add one daily scheduled task (the free plan
   allows one):
   ```bash
   cd ~/value-desk && git pull
   ```
   Schedule it a little after your GitHub Actions run (e.g. if Actions runs
   at 11:00 UTC, schedule this for 11:15 UTC) so it pulls the day's fresh
   `market.db`. No app reload needed — `db.py` opens a new SQLite
   connection per request, so it picks up the new file automatically.
